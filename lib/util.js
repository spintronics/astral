...
export let hashCode = val => {
  var hash = 0
  var type = typeof val
  if (val === null) return val
  if ('number' === type) val = parseFloat(val)
  if ('object' === type) {
    try {
...
export let memoize = (fn, context = null) => {
  let cache = {}
  return (...args) => {
    let hash = hashCode(args)
    return cache[hash] || fn.apply(context, args)
  }
}
